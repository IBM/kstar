    ...
    while (true) {
        ...
        */
        ...
        if (lazy_evaluator) {
            ...
            */
            if (node->is_dead_end()) {
                log << "Node is a dead end." << endl;
                node->dump(task_proxy, log);
                continue;
            }
            ...
            if (lazy_evaluator->is_estimate_cached(s)) {
                ...
                if (open_list->is_dead_end(eval_context)) {
                    ...
                    log << "Node marked as a dead end." << endl;
                    node->dump(task_proxy, log);
    ...
    for (OperatorID op_id : applicable_ops) {
        ...
        */
        ...
        if (succ_node.is_dead_end()) {
            log << "Successor node is a dead end." << endl;
            succ_node.dump(task_proxy, log);
            continue;
        }
        ...
        if (succ_node.is_new()) {
            ...
            if (open_list->is_dead_end(succ_eval_context)) {
                ...
                statistics.inc_dead_ends();
                log << "Successor node is marked as a dead end." << endl;
                succ_node.dump(task_proxy, log);
                continue;
            }
