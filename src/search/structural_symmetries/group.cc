...
Group::Group(const options::Options &opts)
    : stabilize_initial_state(opts.get<bool>("stabilize_initial_state")),
      ...
      keep_state_identity_operator_symmetries(opts.get<bool>("keep_state_identity_operator_symmetries")),
      num_vars(0),
      num_operators(0),
...
const OperatorPermutation &Group::get_operator_permutation(int index) const { 
    if (index < (int) operator_generators.size())  
        return operator_generators[index];
    return operator_state_identity_generators[index - operator_generators.size()];
}

int Group::get_num_operator_symmetries() const {
    if (keep_state_identity_operator_symmetries)
        return (int) (operator_generators.size() + operator_state_identity_generators.size()); 
    return (int) operator_generators.size();
}
...
void Group::add_raw_generator(const unsigned int *generator) {
    ...
    if (permutation.identity()) {
        ...
        if (keep_operator_symmetries && keep_state_identity_operator_symmetries) {
            OperatorPermutation op_permutation(*this, generator);
            operator_state_identity_generators.push_back(move(op_permutation));
        }
    } else {
        if (write_search_generators || write_all_generators) {
            add_to_be_written_generator(generator);
        ...
        if (keep_operator_symmetries) {
            OperatorPermutation op_permutation(*this, generator);
            OperatorPermutation op_inverse_permutation(op_permutation, true);
            operator_generators.push_back(move(op_permutation));
            operator_inverse_generators.push_back(move(op_inverse_permutation));
        }
    }
}
...
void Group::compute_permutation_trace_to_canonical_representative(const State &state, vector<int>& permutation_trace) const {
...
RawPermutation Group::create_permutation_from_state_to_state(
    ...
    assert(has_symmetries());
    vector<int> from_state_permutation_trace;
    compute_permutation_trace_to_canonical_representative(from_state, from_state_permutation_trace);
    vector<int> to_state_permutation_trace; 
    compute_permutation_trace_to_canonical_representative(to_state, to_state_permutation_trace);

    for (int i : from_state_permutation_trace) {
        trace.push_back(make_pair(i, true));
    }
    for (int id = to_state_permutation_trace.size() -1 ; id >= 0; --id) {
        trace.push_back(make_pair(to_state_permutation_trace[id], false));
    }
...
int Group::permute_operator(int op_id, const PermutationTrace& trace) const {
    int curr_op_id = op_id;

    for ( auto id : trace) {
        if (id.second) {
            curr_op_id = operator_generators[id.first].get_permuted_operator_no(curr_op_id);
        } else {
            curr_op_id = operator_inverse_generators[id.first].get_permuted_operator_no(curr_op_id);
        }
    }
    return curr_op_id;
}
...
static shared_ptr<Group> _parse(OptionParser &parser) {
    ...
    parser.add_option<bool>("keep_operator_symmetries",
                            ...
                            "false");
    parser.add_option<bool>("keep_state_identity_operator_symmetries",
                            "Compute and keep operator symmetries for generators that are identity on states",
                            "false");
