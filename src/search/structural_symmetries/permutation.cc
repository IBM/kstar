...
void Permutation::dump_pddl(const TaskProxy &task_proxy) const {
    for (int i = 0; i < group.get_permutation_num_variables(); ++i) {
        for (int j = 0; j < task_proxy.get_variables()[i].get_domain_size(); ++j) {
            pair<int, int> var_val = get_new_var_val_by_old_var_val(i, j);
            if (i == var_val.first && j == var_val.second)
                continue;
            
            utils::g_log << task_proxy.get_variables()[i].get_fact(j).get_name() << " -> "
                 << task_proxy.get_variables()[var_val.first].get_fact(var_val.second).get_name() << ",  ";
        }
    }
    utils::g_log << endl;
}
