...
void SearchStatistics::print_detailed_statistics_with_preamble(std::string& preamble) const {
    // to avoid parsed by single-search-parser.py in lab, replaced ' ' with =
    log << preamble << "Expanded=" << expanded_states << " state(s)." << endl;
    log << preamble << "Reopened=" << reopened_states << " state(s)." << endl;
    log << preamble << "Evaluated=" << evaluated_states << " state(s)." << endl;
    log << preamble << "Evaluations:=" << evaluations << endl;
    log << preamble << "Generated=" << generated_states << " state(s)." << endl;
    log << preamble << "Dead ends:=" << dead_end_states << " state(s)." << endl;

    if (lastjump_f_value >= 0) {
        log << preamble << "Expanded until last jump:="
            << lastjump_expanded_states << " state(s)." << endl;
        log << preamble << "Reopened until last jump:="
            << lastjump_reopened_states << " state(s)." << endl;
        log << preamble << "Evaluated until last jump:="
            << lastjump_evaluated_states << " state(s)." << endl;
        log << preamble << "Generated until last jump:="
            << lastjump_generated_states << " state(s)." << endl;
    }
}
