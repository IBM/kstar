...
for (inc_percent_l, inc_percent_u, sg, h) in params:
    hc = HEURISTICS[h]
    CONFIG_NICKS.append(('uq-acss-%s-tr-%s-%s-%s-%s' % (h, exp_q, inc_percent_l, inc_percent_u, sg), 
        ['--search', 'kstar(%s, pruning=limited_pruning(pruning=atom_centric_stubborn_sets(use_sibling_shortcut=true, atom_selection_strategy=quick_skip)), q=%s, openlist_inc_percent_lb=%s, openlist_inc_percent_ub=%s, switch_on_goal=%s, extend_plans_with_symm=false, extend_plans_with_reordering=false, dump_plans=false, find_unordered_plans=true)' % (hc, exp_q, inc_percent_l, inc_percent_u, sg)]))
