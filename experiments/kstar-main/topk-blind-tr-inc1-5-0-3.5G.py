...
params = [(1, 5, "false", "blind")]
for (inc_percent_l, inc_percent_u, sg, h) in params:
    hc = HEURISTICS[h]
    CONFIG_NICKS.append(('topk-%s-tr-%s-%s-%s-%s' % (h, exp_k, inc_percent_l, inc_percent_u, sg), 
        ['--search', 'kstar(%s, k=%s, openlist_inc_percent_lb=%s, openlist_inc_percent_ub=%s, switch_on_goal=%s, extend_plans_with_symm=false, extend_plans_with_reordering=false)' % (hc, exp_k, inc_percent_l, inc_percent_u, sg)]))
